...
struct proc findmin(){
    ...
    while(temp!= NULL)
    {
        if(min.rbt > temp->node.rbt && temp->node.rbt != 0){
            min = temp->node; 
        }
        temp = temp->next;
    }
...
void srtf(struct proc arr[]){
    ...
    while(rare != 0){
        ...
        for(int i =0;i<pc;i++){
            printf("%d -> %d     ",arr[i].pid,arr[i].rbt);
        }
...
void processStatics(){
    ...
    printf("----------------------------------------------------\n");
    printf("PID     AT      BT      WT      TAT     CT      RT\n");
    printf("----------------------------------------------------\n");
    for(int i =0;i<pc;i++){
        printf("%d%8d%8d%9d%8d%8d%8d",processes[i].pid ,processes[i].at,processes[i].bt,processes[i].wt,processes[i].tat,processes[i].ct ,responsetime[i]);
        printf("\n");
    }
    printf("----------------------------------------------------\n");
...
int main(){
    ...
    do{
        ...
        switch(option){
            ...
            case 2:
                printf("\n\n Remaining Burst Time after one iteration \n\n");
                for(int i =0;i<pc;i++){
                    printf("%d -> %d     ",arr[i].pid,arr[i].rbt);
                }
                printf("\n\n");
